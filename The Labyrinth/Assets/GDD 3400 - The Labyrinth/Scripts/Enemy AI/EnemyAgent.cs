...
namespace GDD3400.Labyrinth
{
    ...
    public class EnemyAgent : MonoBehaviour
    {
        ...
        [SerializeField] private float _RoamInterval = 2f;
        ...
        public void Start()
        {
            ...
            if (_player == null)
                Debug.LogWarning("Player not assigned! Please tag the player as 'Player'.");
        }
        ...
        private void Perception()
        {
            ...
            //chase the player when the player gets into detection radius
            if (distanceToPlayer <= _DetectionRadius)
            {
                _currentState = EnemyState.Chasing;
            }
            else if (distanceToPlayer > _DetectionRadius && _currentState == EnemyState.Chasing)
            {
        ...
        private void DecisionMaking()
        {
            ...
            switch (_currentState)
            {
                ...
                case EnemyState.Roaming:
                    if (Vector3.Distance(transform.position, _floatingTarget) < _StoppingDistance)
                    {
                        // Reached roam point, wait a bit
                        _nextRoamTime = Time.time + _RoamInterval;
        ...
        /// <summary>
        /// picks a random point for enemy to raom to using the roaming radius
        /// </summary>
        private void PickRandomRoamTarget()
        {
            // Pick a random point within the roam radius around the within set radius
        ...
        private void PathFollowing()
        {
            ...
            int nextNodeIndex = Mathf.Min(closestNodeIndex + 1, _path.Count - 1);
            PathNode targetNode = _path[nextNodeIndex];
